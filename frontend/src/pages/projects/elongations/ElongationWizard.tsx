...
import { ReaderStatus, TheoryActions, ScanReadingReview } from './ElongationReaderTools';
...
function TheoryReview({ companyId, projectId, job, refresh }: { companyId: string; projectId: string; job: ElongationJobV2; refresh: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <TheoryActions companyId={companyId} projectId={projectId} job={job} refresh={refresh} />
      <Button size="sm" variant="outline" disabled={!canEdit || readingBusy(job)} onClick={() => setSelected(selected.length === job.items.length ? [] : job.items.map(item => item.id))}>{selected.length === job.items.length ? "Quitar selección" : "Seleccionar todas para clasificar"}</Button>
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {visibleItems.map((item) => <TheoryCard key={`${item.id}-${item.label}-${item.length_m}-${item.strand_count}-${item.calculated_elongation}`} item={item} selected={selected.includes(item.id)} canEdit={canEdit && !readingBusy(job)} canApprove={canApprove && !readingBusy(job)} onToggle={() => toggle(item.id)} onPatch={(patch) => applyPatch(item.id, patch)} />)}
      </div>
...
function MeasurementsStep({ companyId, projectId, job, refresh }: { companyId: string; projectId: string; job: ElongationJobV2; refresh: () => void }) {
  ...
  return <Card><CardHeader><CardTitle>5. Mediciones reales</CardTitle><CardDescription>Cargue varias fotos, PDF o escaneos. La lectura visual propone números manuscritos y los vincula al cable correspondiente. Las lecturas siempre se revisan antes de aprobar.</CardDescription></CardHeader><CardContent className="space-y-4"><ReaderStatus companyId={companyId} projectId={projectId} /><ScanReadingReview companyId={companyId} projectId={projectId} job={job} refresh={refresh} /><div className="rounded-md bg-muted/60 p-3 text-sm">Se lee la página completa y sus recortes. Las cifras impresas de Elong, las cotas y la cantidad S no se usan como mediciones reales. Si una cifra o su asociación es dudosa, se indica para revisión.</div><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><div className="flex-1 space-y-1"><Label htmlFor="measurement-files">Fotos o PDF</Label><Input ref={fileRef} id="measurement-files" type="file" multiple accept="application/pdf,image/jpeg,image/png,image/webp" disabled={!canEdit || !job.theory_approved_at} onChange={(event) => setFiles(Array.from(event.target.files ?? []))} /><p className="text-xs text-muted-foreground">{files.length ? `${files.length} archivo(s) listo(s)` : 'Puede seleccionar varios archivos.'}</p></div><Button disabled={!canEdit || !job.theory_approved_at || files.length === 0 || upload.isPending || readingBusy(job)} onClick={startUpload}>{upload.isPending ? <Loader2 className="animate-spin" /> : <Upload />} Analizar mediciones</Button></div>{!job.theory_approved_at ? <p className="text-sm text-amber-700">La carga se habilita cuando la teoría haya sido aprobada técnicamente.</p> : null}</CardContent></Card>;
}
